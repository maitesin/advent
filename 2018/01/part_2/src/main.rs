...
fn calculate_frenquency(input: String) -> i32 {
    let mut steps: Vec<Step> = Vec::new();
    for step in input.split(" ") {
        steps.push(Step{direction:&step[0..1], distance:step[1..].parse::<i32>().unwrap()});
    }
    let mut result : i32 = 0;
    let mut found: Vec<i32> = vec![0];
    loop {
        for step in &steps {
            if step.direction == "+" {
...
mod tests {
    ...
    #[test]
    fn test_calculate_frequency_for_0(){
        let input = String::from("+1 -1");
        assert_eq!(calculate_frenquency(input), 0);
    }

    #[test]
    fn test_calculate_frequency_for_10(){
        let input = String::from("+3 +3 +4 -2 -4");
        assert_eq!(calculate_frenquency(input), 10);
    }

    #[test]
    fn test_calculate_frequency_for_5(){
        let input = String::from("-6 +3 +8 +5 -6");
        assert_eq!(calculate_frenquency(input), 5);
    }

    #[test]
    fn test_calculate_frequency_for_14(){
        let input = String::from("+7 +7 -2 -7 -4");
        assert_eq!(calculate_frenquency(input), 14);
    }
}
